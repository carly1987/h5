...
{
	if(displayObject._div)displayObject._div.style.display = displayObject.visible ? "block" : "none";
	...
	//		this.context.setTransform(transform[0], transform[3], transform[1], transform[4], transform[2], transform[5])
		...
		//	console.log("matrix(" + transform + ")")
			displayObject._div.style["-webkit-transform"] = "matrix(" + transform[0] + ", " +transform[3]+","+ transform[1] + "," +  
													transform[4] +"," + ( transform[2] + displayObject.anchor.x * -frame.width )+ "," + ( transform[5] + displayObject.anchor.y * -frame.height)+ ")";
...
{
	var isSprite = displayObject instanceof PIXI.Sprite;
	if(!isSprite)return;
	...
	div.src = displayObject.texture.baseTexture.imageUrl;
...
{
	if(!displayObject instanceof PIXI.Sprite)return;
	if(!displayObject._div)return;
	this.view.removeChild(displayObject._div);
}
